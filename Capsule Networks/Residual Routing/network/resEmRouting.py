...
class ConvCaps(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.w_shared:
            ...
            #print('out after merging ', out.shape)
...
class CapsNet(nn.Module):
    ...
    def forward(self, x, y, recon=True, probe=False):
        x = self.conv1(x)
        x = self.bn1(x)
        #x = self.convd(x)
        x = self.relu1(x)
        x = self.primary_caps1(x)
        x, x1 = self.conv_caps1_1(x)
        x = x.permute(0, 2, 3, 1)
        x, x1 = self.conv_caps1_2(x)
        x = self.bn2(x)
        x = self.primary_caps2(x)
        x, x1 = self.conv_caps2_1(x)
        x = x.permute(0, 2, 3, 1)
        x, x1 = self.conv_caps2_2(x)
        x = self.bn3(x)
        x = self.primary_caps3(x)
        x, x1 = self.conv_caps3_1(x)
        x = x.permute(0, 2, 3, 1)
        x, x1 = self.conv_caps3_2(x)
        x = self.bn4(x)
        x = self.primary_caps4(x)
        x, x1 = self.conv_caps4_1(x)
        x = x.permute(0, 2, 3, 1)
        x, x1 = self.conv_caps4_2(x)    
        x = self.bn5(x)
        x = self.primary_caps5(x)
        out = self.class_caps(x) # torch.Size([32, 10])

        p= out[1]
    
        if recon:
            target_pose = p[torch.arange(0, p.shape[0], device=out[0].device), y, :]
            ...
            return out[0], out[1], recon
        else:
            # If reconstruction is not enabled, return just the class activations
